    ...
    <nav className={`fixed top-0 z-50 w-full navbar-glass ${scrolled ? 'scrolled' : ''} transition-all duration-300`}>
      <div className="flex justify-between items-center py-3 px-6 md:px-16 lg:px-32">
        ...
        <div className="hidden lg:flex gap-10 xl:gap-14 items-center">
          {navLinks.map((link) => (
            <a key={link.href} href={link.href} className="text-black font-pjs font-semibold text-base xl:text-lg hover:text-[#FCC61D] transition-all duration-300 cursor-pointer">
              {link.label}
            </a>
          ))}
          <button className="bg-[#FCC61D] text-[#f7f7f7] rounded-3xl font-pjs font-bold text-base xl:text-lg px-8 xl:px-12 py-2.5 shadow-[0_5px_0_#C59560] hover:bg-[#C59560] hover:shadow-none hover:translate-y-1 transition-all duration-300 cursor-pointer">
            Mulai
          </button>
        </div>
      ...
      <div className={`lg:hidden mobile-menu ${isOpen ? 'open' : ''}`}>
        <div className="flex flex-col items-center gap-4 py-6 px-6 bg-white/95 border-t border-gray-100">
          ...
          <button className="bg-[#FCC61D] text-[#f7f7f7] rounded-3xl font-pjs font-bold text-lg px-12 py-2.5 shadow-[0_5px_0_#C59560] hover:bg-[#C59560] hover:shadow-none hover:translate-y-1 transition-all duration-300 cursor-pointer mt-2">
            Mulai
          </button>
        </div>
      </div>
