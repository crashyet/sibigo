  ...
  return (
    <div className="relative overflow-hidden min-h-screen w-full px-4 md:px-24">
      ...
      <div className="flex flex-col items-center pt-24 md:pt-18 px-4 w-full">
        ...
        <p className="text-gray-500 text-base md:text-lg mb-12 md:mb-24 text-center max-w-2xl">
          Pelajari huruf SIBI dari tingkat dasar hingga mahir.
        </p>

        {/* Section Title with Icon */}
        <div className="w-full max-w-5xl flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 mb-8 md:mb-12 px-2">
          ...
          <div className="flex flex-col items-center md:items-start">
            <h3 className="text-2xl md:text-3xl font-bold text-[#2D2D2D]">Imbuhan</h3>
            <span className="text-gray-500 font-medium text-base md:text-lg">3 pelajaran</span>
          </div>
        </div>

        {/* Lesson Cards */}
        <div className="relative w-full max-w-5xl flex flex-col gap-4 md:gap-6 pb-12">
          {lessons.map((lesson, index) => (
            <Link
              to={`/lesson/words/${lesson.slug}`}
              key={index}
              className={`${lesson.bgColor} ${lesson.shadowColor} w-full rounded-[24px] md:rounded-[28px] p-4 md:p-6 flex flex-col sm:flex-row items-center justify-between text-white hover:scale-[1.01] active:scale-[0.98] transition-all duration-300 cursor-pointer group text-left gap-4`}
            >
              <div className="flex items-center gap-4 md:gap-5 w-full sm:w-auto">
                <div className="w-12 h-12 md:w-14 md:h-14 bg-white backdrop-blur-md rounded-xl md:rounded-2xl flex items-center justify-center p-2 md:p-3 shrink-0">
                  {/* Scale SVG for mobile */}
                  <div className="scale-75 md:scale-100 flex items-center justify-center">
                    {lesson.svgIcon}
                  </div>
                </div>
                <div className="flex flex-col">
                  <h4 className="text-lg md:text-xl font-bold">{lesson.range}</h4>
                  <p className="text-white/80 text-sm md:text-lg font-medium line-clamp-1">{lesson.description}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 text-base md:text-lg font-medium text-white/90 self-end sm:self-center">
